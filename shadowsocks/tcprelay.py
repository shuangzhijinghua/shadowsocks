...
class TCPRelayHandler(object):
    ...
    def _handle_stage_addr(self, ogn_data, data):
        try:
            ...
            common.connect_log('%s connecting %s:%d via port %d from %s:%d' %
                        ((connecttype == 0) and 'TCP' or 'UDP',
                            common.to_str(remote_addr), remote_port, self._server._listen_port,  self._client_address[0], self._client_address[1]))
